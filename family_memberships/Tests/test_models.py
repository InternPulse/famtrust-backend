...
class FamilyGroupModelTests(TestCase):
    ...
    def test_family_group_creation(self):
        ...
        self.assertEqual(
            self.family_group.description, "This is a test family group."
        )
...
class FamilyMembershipModelTests(TestCase):
    ...
    def test_family_membership_creation(self):
        ...
        self.assertEqual(
            self.family_membership.family_group, self.family_group
        )
        self.assertIsInstance(
            self.family_membership.joined_at, timezone.datetime
        )
        self.assertIsInstance(self.family_membership.id, uuid.UUID)

    def test_str_method(self):
        expected_str = (
            f"Family Membership {self.family_membership.id} for"
            f" {self.user} in {self.family_group}"
        )
