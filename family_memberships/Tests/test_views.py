...
class FamilyGroupViewSetTests(TestCase):
    ...
    def test_create_family_group(self):
        ...
        response = self.client.post(
            self.family_group_url, data, format="json"
        )
...
class FamilyMembershipViewSetTests(TestCase):
    ...
    def test_create_family_membership(self):
        data = {"user": self.user.id, "family_group": self.family_group.id}
        response = self.client.post(
            self.family_membership_url, data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_retrieve_family_membership(self):
        url = f"{self.family_membership_url}{self.family_membership.id}/"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
